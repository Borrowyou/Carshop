            ...
            for (int Index = 1; Index < AllModels.length; Index++)
            {
                ...
                /*if (NumberOfThreads < 2)
                {
                    var th = new Thread(() =>
                    {
                        ...
                        LinkExtractor LinkExtr = new LinkExtractor(CurrentLink, null);
                        LinkExtr.SearchPartPage();
                    });
                ...
                foreach (HtmlNode row in TableElem.ChildNodes)
                {
                    ...
                    if (FoundLinks.Count != 0)
                        foreach (var Link in FoundLinks)
                            DSCarShop.PartLinkTblAdapter.Insert(DSCarShop.GEN_ID("PART_LINKS"), DSCarShop.GEN_ID("PART_ID"), Constants.AutoBimLink + Link, DateTime.Now, 1);
